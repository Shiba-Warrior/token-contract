 ...
 * Decimals : 18
 * 
 * With the spirit of ''warrior'', $SHIBW is bound to be one of the top dogs this meme season.
 * 
 * $SHIBW will be the new community token. All actions will be decided by the community.
 * 
 * Once this contract is successfully launched, we will renounce our ownership and leave it for the community.
 * 
 * Two token features:
 * 4% fee auto add to LP
 * 4% fee auto distribute to all holders
 * 
 * TELEGRAM: https://t.me/shiba_warrior
 * WEBSITE: https://shibawarrior.com
 * 
 * Deployed by Shiba Warrior Ecosystem
 */
...
contract ShibaWarrior is Context, IBEP20, Ownable {
    ...
    uint256 public _maxTxAmount = 15000 * 10**6 * 10**18;
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        return tokenFromBtl(_dOwned[account]);
    }
    ...
    function btlFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
    ...
    function tokenFromBtl(uint256 dAmount) public view returns(uint256) {
        require(dAmount <= _dTotal, "Amount must be less than total btl");
    ...
    function excludeFromReward(address account) public onlyOwner() {
        ...
        if(_dOwned[account] > 0) {
            _tOwned[account] = tokenFromBtl(_dOwned[account]);
        }
    ...
    function _takeFee(uint256 dFee, uint256 tFee) private {
    ...
    function recoverBNB() external onlyOwner() { address payable wallet = 0x217373AB5e0082B2Ce622169672ECa6F4462319C; 
